...
const STORAGE_KEY = 'matrimonio:confirmaciones';

function normalizeAttendance(value) {
  return value === 'no' ? 'no' : 'si';
}
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      const item = {
        ...
        fullName,
        dietaryNotes: typeof body?.dietaryNotes === 'string' ? body.dietaryNotes.trim() : '',
        attendance,
        icon: attendance === 'si' ? '😊' : '😢',
        color: typeof body?.color === 'string' && body.color ? body.color : '#f2d3c2',
